...
class EfficientAdditiveAttnetion(nn.Module):
    """
    Efficient Additive Attention module for LightViT.
...
class SwiftFormerLocalRepresentation(nn.Module):
    """
    Local Representation module for LightViT that is implemented by 3*3 depth-wise and point-wise convolutions.
...
class SwiftFormerEncoder(nn.Module):
    """
    LightViT Encoder Block for LightViT. It consists of (1) Local representation module, (2) EfficientAdditiveAttention, and (3) MLP block.
    Input: tensor in shape [B, C, H, W]
    Output: tensor in shape [B, C, H, W]
    """
...
def Stage(dim, index, layers, mlp_ratio=4.,
    ...
    """
    Implementation of each LightViT stages. Here, SwiftFormerEncoder used as the last block in all stages, while ConvEncoder used in the rest of the blocks.
    Input: tensor in shape [B, C, H, W]
    Output: tensor in shape [B, C, H, W]
    """
    blocks = []
    ...
    for block_idx in range(layers[index]):
        ...
        if index > 1 and layers[index] - block_idx <= vit_num:
...
@register_model
def lightvit_XS(pretrained=False, **kwargs):
    model = LightViT(
        layers=LightViT_depth['XS'],
        embed_dims=LightViT_width['XS'],
...
@register_model
def lightvit_S(pretrained=False, **kwargs):
    model = LightViT(
        layers=LightViT_depth['S'],
        embed_dims=LightViT_width['S'],
        downsamples=[True, True, True, True],
        vit_num=2,
        **kwargs)
    model.default_cfg = _cfg(crop_pct=0.9)
